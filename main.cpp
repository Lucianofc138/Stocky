...
int main(int argc, char **argv)
{
    ...
    cv::Mat roi1 = imread("samples/samp0a.jpeg", IMREAD_COLOR);
    cv::Mat roi2 = imread("samples/samp0b.jpeg", IMREAD_COLOR); 
...
float* getChromaHistogram(cv::Mat BGRimg, const int bins) {
    ...
    uchar channelHue, channelSat, channelLum;
    ...
    for(int i=0; i<h; i++){
        for(int j=0; j<w; j++){
            ...
            channelLum = channels.at(1).at<unsigned int>(i, j);
            if (channelLum < 205 && channelLum > 50){ //condicion de Luma entre minimo y maximo ej: entre 30 y 220
